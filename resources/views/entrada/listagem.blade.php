            ...
            <tbody>
                @foreach ($produtos as $p)
                    <tr>
                        ...
                        <td id="imagem">{!! $p->imagens ? "<img width=\"90\" src=\"$p->imagens\">" : 'Sem Foto' !!}</td>
                        <td>{{ $p->descricao }}</td>
                        <td>R${{ number_format($p->valor, 2, ',', '.') }}</td>
                        <td>{{ date('d/m/Y', strtotime($p->created_at)) }}</td>
                        <td>{{ date('H:i:s', strtotime($p->created_at)) }}</td>
                        <td>{{ $p->quantidade }}</td>
                        <td><a href="/ListarEntrada/mostrar/{{ $p->id_entrada }}"><span
                                    class="glyphicon glyphicon-pencil"></span></a></td>
                        <td><a href="/ListarEntrada/remove/{{ $p->id_entrada }}"><span
